...
# Environment variables are optional for this frontend-only app
```
...
```bash
# .env.local (optional for customization)
...
```typescript
// libs/json-formatter.ts
export function formatJson(data: string, minify: boolean) {
...
```typescript
// providers/ToolStateProvider.tsx
