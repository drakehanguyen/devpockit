...
export function CronBuilder({ className }: CronBuilderProps) {
  const { toolState, updateToolState } = useToolState('cron-builder');

  // Initialize with defaults to avoid hydration mismatch
  const [options, setOptions] = useState<CronBuilderOptions>(DEFAULT_CRON_BUILDER_OPTIONS);
  ...
  useEffect(() => {
    setIsHydrated(true);
    if (toolState) {
      if (toolState.options) setOptions(toolState.options as CronBuilderOptions);
  ...
  useEffect(() => {
    if (isHydrated && (!toolState || Object.keys(toolState).length === 0)) {
      setOptions(DEFAULT_CRON_BUILDER_OPTIONS);
      setExpression('');
      setPreview('');
      setError('');
  ...
  const updateField = (fieldName: keyof CronBuilderOptions, value: CronFieldValue) => {
    setOptions(prev => ({
      ...prev,
      [fieldName]: value
    }));
    ...
    <div className={cn('flex flex-col h-full', className)}>
      ...
      <div className="bg-background px-[28px] pt-[36px] pb-[20px]">
        ...
        <p className="text-sm leading-5 tracking-normal text-neutral-900 dark:text-neutral-100">
          Build cron expressions visually with a step-by-step form builder. Get real-time validation and preview.
        </p>
      </div>
